...
class Problem: 
    def __init__(self, H, B, n_nodes): 
        ...
        self.cross_costs = np.random.rand(n_nodes, n_nodes)
        self.cross_costs = (self.cross_costs + self.cross_costs.T) / 2
        for i in range(n_nodes): self.cross_costs[i,i] = 0

        self.cross_costs_ordered = [(self.cross_costs[i,j], (i,j)) for i in range(n_nodes) for j in range(n_nodes)]
        self.cross_costs_ordered = sorted(
            self.cross_costs_ordered,
            key=lambda x: x[0]
        )
        self.cross_costs = torch.tensor(self.cross_costs)
